...
2. **保留决策动机 (Decision Reasoning)**：对于每一个关键步骤，必须说明「为什么要这样做」以及「遇到了什么坑才这样选择」。这对于未来复用至关重要。
3. **SKILL.md 结构**：
   - `name`: 机器可读的唯一标识（kebab-case）。
   - `description`: 简单说明此技能解决什么问题。
   - `instructions`: 核心部分。列出关键步骤、必要的工具调用、避坑指南、以及每一步背后的决策动机。
4. **验证点**：指出在执行过程中，如何判断每一步是否成功。
